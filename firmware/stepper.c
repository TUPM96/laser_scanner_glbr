/*
  stepper.c - Implementation driver stepper motor
  
  CHỨC NĂNG:
  - Thực thi motion plans từ planner bằng stepper motors
  - Quản lý interrupt timer để tạo xung step chính xác
  - Xử lý acceleration, deceleration, và cruise
  - Adaptive Multi-Axis Step Smoothing (AMASS)
  - Bresenham algorithm cho multi-axis stepping
...
// Hằng số hữu ích
#define DT_SEGMENT (1.0/(ACCELERATION_TICKS_PER_SECOND*60.0)) // Thời gian mỗi segment (phút)
#define REQ_MM_INCREMENT_SCALAR 1.25                           // Hệ số tăng increment
#define RAMP_ACCEL 0   // Chế độ tăng tốc
#define RAMP_CRUISE 1  // Chế độ tốc độ ổn định
#define RAMP_DECEL 2   // Chế độ giảm tốc

// Adaptive Multi-Axis Step Smoothing (AMASS) - làm mượt stepping đa trục
// Mỗi level có cutoff frequency riêng, level cao hơn overdrive ISR nhiều hơn
// Level 0 (không AMASS) bắt đầu từ cutoff frequency của Level 1
// LƯU Ý: Cutoff frequency * ISR overdrive factor không được vượt quá max step frequency
// Cài đặt hiện tại: overdrive ISR tối đa 16kHz để cân bằng CPU overhead và độ chính xác timer
#define MAX_AMASS_LEVEL 3
#define AMASS_LEVEL1 (F_CPU/8000) // Overdrive ISR x2
#define AMASS_LEVEL2 (F_CPU/4000) // Overdrive ISR x4
#define AMASS_LEVEL3 (F_CPU/2000) // Overdrive ISR x8
