/*
  motion_control.c - Implementation giao diện cấp cao cho lệnh chuyển động
...
// LƯU Ý: Đây là cổng chính vào planner GRBL. Tất cả chuyển động linear, kể cả arc segments,
// phải đi qua hàm này trước khi vào planner. Tách mc_line và plan_buffer_line để tách
// các hàm không phải planner và cho phép backlash compensation hoặc canned cycle tích hợp dễ dàng
...
#endif
{
  // Kiểm tra soft limit violations nếu bật (đặt ở đây để bắt tất cả line motions)
  if (bit_istrue(settings.flags,BITFLAG_SOFT_LIMIT_ENABLE)) { limits_soft_check(target); }    
      
  // Nếu ở chế độ check gcode, chặn motion bằng cách block planner (soft limits vẫn hoạt động)
  if (sys.state == STATE_CHECK_MODE) { return; }
    
  // Nếu buffer đầy: tốt! Nghĩa là chúng ta đã đi trước robot 
