/*
  planner.c - Implementation planner chuyển động
  
  CHỨC NĂNG:
  - Buffer lệnh chuyển động và quản lý acceleration profile
  - Tính toán tốc độ tại junction
  - Tối ưu hóa đường đi và tốc độ
  - Quản lý block buffer cho chuyển động linear và arc
...
#define SOME_LARGE_VALUE 1.0E+38 // Giá trị lớn cho tính toán rapids và acceleration maximization
                                 // Phải lớn hơn bất kỳ giá trị (mm/min)^2 hoặc mm/sec^2 khả thi

// Buffer block (ring buffer) cho lệnh chuyển động
static plan_block_t block_buffer[BLOCK_BUFFER_SIZE];
static uint8_t block_buffer_tail;     // Index block đang xử lý
static uint8_t block_buffer_head;     // Index block tiếp theo được push
static uint8_t next_buffer_head;      // Index buffer head tiếp theo
static uint8_t block_buffer_planned;  // Index block đã được tối ưu hóa

// Struct planner variables
typedef struct {
  int32_t position[N_AXIS];          // Vị trí planner của tool trong steps tuyệt đối
                                     // Tách biệt với vị trí g-code cho chuyển động nhiều dòng
  float previous_unit_vec[N_AXIS];   // Vector đơn vị của đoạn đường trước
  float previous_nominal_speed_sqr;  // Tốc độ nominal của đoạn đường trước (mm/min)^2
} planner_t;
static planner_t pl;
