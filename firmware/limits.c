/*
  limits.c - Implementation module giới hạn và homing cycle
...
// Hệ số nhân khoảng cách tìm kiếm khi homing (tính bằng giá trị này nhân với cycle travel)
#ifndef HOMING_AXIS_SEARCH_SCALAR
  #define HOMING_AXIS_SEARCH_SCALAR  1.5 // Phải > 1 để đảm bảo limit switch sẽ được kích hoạt
#endif
#ifndef HOMING_AXIS_LOCATE_SCALAR
  #define HOMING_AXIS_LOCATE_SCALAR  5.0 // Phải > 1 để đảm bảo limit switch được clear
#endif

// Khởi tạo module limits
void limits_init() 
{
  LIMIT_DDR &= ~(LIMIT_MASK); // Cấu hình làm input pins

  #ifdef DISABLE_LIMIT_PIN_PULL_UP
    LIMIT_PORT &= ~(LIMIT_MASK); // Normal low - cần pull-down ngoài
  #else
    LIMIT_PORT |= (LIMIT_MASK);  // Bật pull-up nội - normal high
  #endif

  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
    LIMIT_PCMSK |= LIMIT_MASK; // Bật các pin cụ thể cho Pin Change Interrupt
    PCICR |= (1 << LIMIT_INT); // Bật Pin Change Interrupt
  } else {
    limits_disable(); // Tắt limits nếu không bật
  }
  
  #ifdef ENABLE_SOFTWARE_DEBOUNCE
    MCUSR &= ~(1<<WDRF);
    WDTCSR |= (1<<WDCE) | (1<<WDE);
    WDTCSR = (1<<WDP0); // Thiết lập timeout ~32ms cho watchdog debounce
  #endif
}
