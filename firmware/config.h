/*
  config.h - File cấu hình compile-time của GRBL
  
  CHỨC NĂNG:
  - Chứa các cấu hình compile-time cho hệ thống GRBL
  - Cấu hình baud rate, CPU map, lệnh real-time
  - Cấu hình homing cycle, buffer sizes, timing
  - Tùy chỉnh hiệu suất và điều chỉnh cho máy không chuẩn
  
  LƯU Ý:
  - Mọi thay đổi ở đây cần compile lại toàn bộ source code
  - Người dùng thường không cần chỉnh sửa file này
  - Chỉ thay đổi khi cần tối ưu hiệu suất hoặc máy đặc biệt
...
#ifndef config_h
#define config_h
#include "grbl.h" // Tương thích Arduino IDE

// Cấu hình mặc định - dùng khi reset EEPROM (đổi tên trong defaults.h)
#define DEFAULTS_GENERIC

// Tốc độ baud serial
#define BAUD_RATE 115200

// Cấu hình CPU map - GRBL hỗ trợ chính thức Arduino Uno
// Có thể thêm các bộ xử lý khác trong cpu_map.h
#define CPU_MAP_ATMEGA328P // CPU Arduino Uno

// Định nghĩa ký tự đặc biệt cho lệnh real-time
// Các ký tự này được lấy trực tiếp từ serial stream và không được truyền vào parser g-code
// Chọn ký tự không có trong chương trình g-code
// LƯU Ý: Nếu thay đổi, cần cập nhật thủ công thông điệp help trong report.c
#define CMD_STATUS_REPORT '?'  // Báo cáo trạng thái
#define CMD_FEED_HOLD '!'      // Tạm dừng feed
