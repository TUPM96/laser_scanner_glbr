/*
  report.c - Implementation phương thức báo cáo và messaging
  
  CHỨC NĂNG:
  - Giao diện feedback chính cho GRBL
  - Báo cáo trạng thái hệ thống GRBL
  - In thông báo lỗi và alarm
  - Báo cáo vị trí máy và work coordinate
  - Báo cáo thông số g-code và settings
  
  LƯU Ý: Các hàm này chủ yếu được gọi từ protocol.c
  Nếu muốn thay đổi style feedback (ví dụ: JSON), có thể sửa các hàm sau
...
// Xử lý phản hồi protocol chính cho streaming interfaces và human-feedback
// Với mỗi dòng incoming, trả về 'ok' cho lệnh thành công hoặc 'error:' cho lỗi
// Lỗi có thể đến từ g-code parser, settings module, hoặc asynchronously từ critical error
// như hard limit được kích hoạt. Interface nên luôn monitor các phản hồi này
// LƯU Ý: Trong silent mode, tất cả error codes > 0
